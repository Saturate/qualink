...
import { collectCoverageCobertura } from "./coverage-cobertura.js";
...
describe("collectCoverageCobertura", () => {
	it("parses Cobertura format", () => {
		const [doc] = collectCoverageCobertura(coberturaXml, makeMetadata());
...
<coverage lines-valid="3" lines-covered="1" branches-valid="0" branches-covered="0"/>`;
		const [doc] = collectCoverageCobertura(xml, makeMetadata());
	...
	it("throws on invalid XML", () => {
		expect(() => collectCoverageCobertura("<not><valid>", makeMetadata())).toThrow();
	});

	it("throws on unrecognized format", () => {
		const xml = `<?xml version="1.0"?><something/>`;
		expect(() => collectCoverageCobertura(xml, makeMetadata())).toThrow(
